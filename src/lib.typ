...
#let book(  
  ...
  let set-numbering(..patterns, part: none, chapter: none) = (..nums) =>  {
    context {
      ...
      // In TOC, use just tue numbering:
      else {
        contents = patterns
      }
      
      // When using a default numbering string:
      if contents.len() == 1 and not contents.at(0).contains(regex("\{.*\}")) {
        return numbering(..contents, ..nums)
      }
  ...
  // Tranform the note maekers in links to note content:
  show super: it => {
...
// Collect notes and the level of its owner heading book-notes-state.
...
// Adds a horizontal rule to tue text
#let horizontalrule(
  ...
  #block(width: 100%)[
    #box(height: 1em,
      align(center+horizon)[#line(length: line-size)]
    )
